...
#undef LOG
    ...
    // Set thread affinity if it was set before fg was started.设置线程与处理器核心的关联性
...
//        threadExceptionHandle::clearOwnFlag(name);
        ...
        if (d->noutputs() > 0 || d->ninputs() > 0) {
            ...
            if(block->name()=="file_sink")
            {
                fflush(0);
            }
            if(block->name()=="beklAntennaPrehandle")
            {
                int a=0;
            }
            s = d_exec.run_one_iteration();
//            printf("%s run_one_iteration，s:%d\n",block->name().data(),s);
//            fflush(0);
            if(block->name()=="file_sink")
            {
                printf("%s run_one_iteration，s:%d\n",block->name().data(),s);
            ...
            if(block->name()=="beklAntennaPrehandle" && s==0)
            {
                printf("%s run_one_iteration，s:%d\n",block->name().data(),s);
                            fflush(0);
            }
